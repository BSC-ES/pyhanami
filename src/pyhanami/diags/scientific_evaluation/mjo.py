...
class MJOEvaluation:
    ...
    def __init__(self, data_sim, obs_path=config_params.MJO_VARS_PATH, start_year_mjo=None, end_year_mjo=None, 
        ...
        self.start_year_mjo, self.end_year_mjo = data_general.validate_year_range(
            data_sim, start_year_mjo, end_year_mjo, process_name="MJO"
        )
        if (
            self.start_year_mjo < config_params.MJO_START_YEAR
        ...
        self.data_ceof_sim, _, _, self.data_ceof_obs, _, _ = self._prepare_ceof_data(
            ...
            start_year_ref,
            end_year_ref,
            mjo_config.lat_range,
            mjo_config.rolling_window_size,
        ...
        print(
            "\tAbsolute values and bias in MJO activity (mean amplitude and days) per phase computation completed."
            " See attribute `activity_per_phase` for results.",
            flush=True,
        )

        # Prepare data for the power spectra analysis (regrid simulations, if needed, and remove seasonal cycle)
        self.data_spectra_sim, self.data_spectra_obs = self._prepare_spectra_data(
        ...
        self.power_spectra = self._compute_power_spectra(
            mjo_config.seg_size, mjo_config.n_overlap, mjo_config.lat_range
        )
        print(
            "\tWavenumber-frequency power spectra computation completed. See attribute `power_spectra` for results.",
            flush=True,
        )
        ...
        print(
            "\tScalar scores related to power spectra computed. See attribute `power_scores` for results.",
            flush=True,
        )
    ...
    def _prepare_mjo_data(self, data_sim, obs_path):
        ...
        try:
            ...
            raise FileNotFoundError(f"Observations MJO data file not found at '{obs_path}'") from e
        data_obs_vars = data_obs_vars[self.mjo_vars].sel(
    ...
    def _prepare_ceof_data(self, data_sim_mjo, data_obs_mjo, start_year_ref, end_year_ref, lat_range=(-15, 15), 
        ...
        filtered_obs, anom_obs, std_obs = mjo_ceof_funcs.remove_longer_time_scale_components(
            data_obs_mjo,
            start_year_ref,
            end_year_ref,
            lat_range,
        ...
        filtered_sim, anom_sim, std_sim = mjo_ceof_funcs.remove_longer_time_scale_components(
            data_sim_mjo,
            start_year_ref,
            end_year_ref,
            lat_range,
    ...
    def _compute_activity_per_phase(self, threshold=None):
        ...
        phase_counts_sim_on_obs = mjo_ceof_funcs.compute_climatological_phase_counts(
            pcs_sim_on_obs, threshold
        )
        ...
        phase_counts_sim_on_sim = mjo_ceof_funcs.compute_climatological_phase_counts(
            pcs_sim_on_sim, threshold
        )
        ...
        activity_per_phase = xr.merge(
            [
                phase_counts,
                phase_counts_bias.rename(
                    {var: var + "_bias" for var in phase_counts_bias.data_vars}
                ),
            ]
        )
    ...
    def _prepare_spectra_data(self, data_sim_mjo, data_obs_mjo, start_year_ref, end_year_ref, n_harmonics=3):
        ...
        for var_name in self.mjo_vars:
            filtered_obs_one_var = mjo_ceof_funcs.remove_seasonal_cycle(
                data_obs_mjo[var_name], start_year_ref, end_year_ref, n_harmonics
            )
            filtered_obs_all_vars.append(filtered_obs_one_var)

            filtered_sim_one_var = mjo_ceof_funcs.remove_seasonal_cycle(
                data_sim_mjo[var_name], start_year_ref, end_year_ref, n_harmonics
            )
    ...
    def _compute_power_spectra(self, seg_size=96, n_overlap=60, lat_range=(-15, 15)):
        ...
        tasks = [
            (self.data_spectra_obs.sel(variable=var_name), seg_size, n_overlap, lat_range)
            for var_name in self.mjo_vars
        ]
        ...
        tasks = [
            (self.data_spectra_sim.sel(variable=var_name), seg_size, n_overlap, lat_range)
            for var_name in self.mjo_vars
        ]
        with concurrent.futures.ThreadPoolExecutor(max_workers=config_params.MAX_WORKERS_VARS) as executor:
            for idx, value in enumerate(executor.map(mjo_spectrum_funcs.wavenum_freq_analysis_wrapper, tasks)):
    ...
    def _compute_power_scores(self, freq_bounds=None, wavenum_bounds=None, freq_dim="frequency", 
        ...
        eastward_obs = []
        for var_name in self.mjo_vars:
            ratio_obs_one_var, eastward_obs_one_var, _ = (
                mjo_spectrum_funcs.compute_eastward_westward_ratio(
                    self.power_spectra["sym_spec"].sel(variable=var_name, dataset="obs"),
                    freq_bounds,
                    wavenum_bounds,
                    freq_dim,
                    wavenum_dim,
            ...
            if eastward_obs_one_var == 0:
                raise ValueError(
                    f"Eastward power in observations is zero in the selected MJO band for variable '{var_name}',"
        ...
        for i in range(1, n_datasets):
            ...
            eastward_sim = []
            for var_name in self.mjo_vars:
                ratio_sim_one_var, eastward_sim_one_var, _ = (
                    mjo_spectrum_funcs.compute_eastward_westward_ratio(
                        self.power_spectra["sym_spec"].sel(variable=var_name).isel(dataset=i),
                        freq_bounds,
                        wavenum_bounds,
                        freq_dim,
                        wavenum_dim,
        ...
        pwfps = [
            [
                mjo_spectrum_funcs.compute_power_periodicity(
                    self.power_spectra["sym_spec"].sel(variable=var).isel(dataset=i),
                    freq_bounds,
                    wavenum_bounds,
                    freq_dim,
                    wavenum_dim,
                )
                for var in self.mjo_vars
            ]
    ...
    def save_data(self, output_path):
        ...
        print(
            f"Output of CEOF analysis for '{self.obs_name}' saved to '{ceof_obs_path}'.",
            flush=True,
        )
        ...
        print(
            f"Output of CEOF analysis for '{self.sim_name}' projected on itself saved to '{ceof_sim_on_sim_path}'.",
            flush=True,
        )
    ...
    def ceof_plots(self, output_path=None):
        ...
        #     'rlut':  '#377eb8'   # blue
        # }
        labels_linestyles = {self.obs_name: "-", self.sim_name: "--"}
    ...
    def lead_lag_corr_plot(self, output_path=None):
        ...
        datasets = [
            lead_lag_corrs.sel(dataset="obs"),
            lead_lag_corrs.sel(dataset="sim_on_obs"),
            lead_lag_corrs.sel(dataset="sim_on_sim"),
        ]
    ...
    def mean_active_amplitude_plot(self, output_path=None):
        ...
        mean_amp_bar_plot, _ = plot.plot_grouped_bars(
            ...
            x_label="MJO phase",
            y_label="mean amplitude per year",
    ...
    def activity_per_phase_plots(self, output_path=None, layout="separate"):
        ...
        if layout == "separate":
            mean_amp_active_days_plot, _ = plot.plot_two_grouped_bars(
                ...
                title_2="Climatological active MJO days per phase",
                x1_label=x_label,
                x2_label=x_label,
                y1_label="mean amplitude per year",
                y2_label="number of active days per year",
                labels=labels,
            )
            ...
            mean_amp_active_days_plot, _ = plot.plot_grouped_bars_two_axes(
                ...
                data_active_days,
                x_values=x_values,
                x_values_minor=x_values_minor,
                title=f"Climatological MJO activity per phase ({year_range})",
                x_label=x_label,
                y1_label="mean amplitude per year",
                y2_label="number of active days per year",
                labels=labels,
    ...
    def power_spectrum_plots(self, output_path=None, spectrum_var='rlut', component='symmetric', 
        ...
        if spectrum_var not in self.mjo_vars:
            raise ValueError(
                f"Variable '{spectrum_var}' not valid for the spectrum plots. Choose one of {self.mjo_vars}."
            )
        ...
        power_spectrum_plot, _ = plot.plot_power_spectrum_two(
            ...
            x_lim=x_lim,
            y_lim=y_lim,
            title_1=self.obs_name,
            title_2=self.sim_name,
            suptitle=f"{component.capitalize()} '{spectrum_var}' power spectrum ({year_range})",
            levels=levels,
            mjo_box=mjo_box,
            mjo_freq_bounds=self.mjo_freq_bounds,
