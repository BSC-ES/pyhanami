...
class ISOEvaluation:
    ...
    def __init__(self, data_sim, var_name="rlut", start_year_eeof=None, end_year_eeof=None,
        ...
        if self.obs:
            ...
            self.start_year_eeof, self.end_year_eeof = data_general.validate_year_range(
                data_sim, start_year_eeof, end_year_eeof, process_name="EEOF"
            )
        ...
        self.start_year_pc, self.end_year_pc = data_general.validate_year_range(
            data_sim, start_year_pc, end_year_pc, process_name="PC"
        )
        print(
            f"\tYears selected for PCs computation: {self.start_year_pc}-{self.end_year_pc}.",
            flush=True,
        )
        ...
        if self.obs:
            self.obs_name = "NOAA"
            data_sim.data, self.eeof_summer, self.eeof_winter, self.pcs_obs = (
        ...
        data_unfiltered_sim = (
            data_sim.data[var_name].sortby("lat").sel(lat=slice(*iso_config.lat_range)).compute()
        )
        data_filtered_sim = iso_scores.apply_lanczos_bandpass_filter(
            data_unfiltered_sim, iso_config.window_size, iso_config.low_freq, iso_config.high_freq
        )
        print("\tSimulation data filtered for ISO timescales.", flush=True)

        if not self.obs:
            # Compute EEOFs from simulation data
            eeof_summer, eeof_winter = self._compute_EEOFs(
                data_filtered_sim, iso_config.lag, iso_config.n_lags, iso_config.n_modes
            )
            self.pcs_obs = None
        ...
        (
            ...
            self.scores["sigma"],
            self.scores["TSS"],
        ) = self._compute_freq_and_scores()
        print(
            f"\tMean monthly frequency computed between {self.start_year_pc} and {self.end_year_pc}."
            " See attributes `freq_sim` (and `freq_obs` if `obs=True`) for results.",
            flush=True,
        )
        ...
        if self.obs:
            print(
                ...
                f"\n\t\tTaylor Skill Score (TSS): {self.scores['TSS']:.2f}",
                flush=True,
            )
    ...
    def _load_and_regrid_obs_data(self, data_sim, obs_path=None):
        ...
        try:
            ...
            raise FileNotFoundError(
                f"NOAA grid file not found at '{config_params.NOAA_GRID_PATH}'."
            ) from e
        ...
        if sim_resolution < obs_resolution:
            ...
            print(
                f"\tSimulation data regridded to match observations' resolution (~{obs_resolution:.2f}°)."
            )
        ...
        # Regrid observations if their resolution is higher
            ...
            print(
                f"\tObservational data regridded to match simulations' resolution (~{sim_resolution:.2f}°)."
            )

        else:
            raise ValueError(
                f"Cannot compare resolutions: sim_resolution={sim_resolution} and obs_resolution={obs_resolution}."
            )
    ...
    def _compute_EEOFs(self, data_sim, lag=5, n_lags=3, n_modes=2):
        ...
        eeof_summer = iso_scores.perform_EEOF_analysis(
            data_eeof_sim,
            self.start_year_eeof,
            self.end_year_eeof,
            "boreal_summer",
            lag,
            n_lags,
        ...
        eeof_winter = iso_scores.perform_EEOF_analysis(
            data_eeof_sim,
            self.start_year_eeof,
            self.end_year_eeof,
            "boreal_winter",
    ...
    def _compute_PCs(self, data_sim):
        ...
        data_pcs_sim = data_sim.sel(
            time=slice(f"{self.start_year_pc}-01-01", f"{self.end_year_pc}-12-31")
        )
        ...
        if self.obs:
            ...
            if self.correct_pc:
                ...
                print(
                    f"\tSimulated PCs have been adjusted using the '{self.obs_name}' observations.",
                    flush=True,
                )
    ...
    def save_data(self, output_path):
        ...
        print(
            f"EEOFs computed from '{name}' for boreal summer saved to '{eeof_summer_path}'.",
            flush=True,
        )
        ...
        print(
            f"EEOFs computed from '{name}' for boreal winter saved to '{eeof_winter_path}'.",
            flush=True,
        )
        ...
        if self.obs:
            ...
            print(
                f"PCs (bimodal ISO indices) computed for '{self.sim_name}' simulations saved to '{pcs_sim_path}'.",
                flush=True,
            )
        ...
        if self.obs:
            ...
            print(
                f"Mean monthly frequency of ISO events computed for '{self.sim_name}' saved to '{freq_sim_path}'.",
                flush=True,
            )
    ...
    def pc_plots(self, output_path=None, years=None, data="sim"):
        ...
        if data == "sim":
            ...
            if not self.obs:
                raise ValueError(
                    "Observational PCs are not available. "
            ...
            raise ValueError("Invalid value for 'data' parameter. Use 'sim' or 'obs'.")
    ...
    def freq_plot(self, output_path=None):
        ...
        freq_plot, _ = plots_iso.plot_freq_ISO(
            ...
            corr=self.scores["R"],
            sigma=self.scores["sigma"],
            tss=self.scores["TSS"],
            title=plot_title,
            sim_label=self.sim_name,
