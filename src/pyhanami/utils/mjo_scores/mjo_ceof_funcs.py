...
def remove_longer_time_scale_components(data, start_year_ref, end_year_ref, lat_range=(-15, 15),
    ...
    filtered_data.attrs = {
        "description": (
            "Preprocessed data with seasonal cycle removed, latitude averaged "
    ...
    anom_data.attrs = {
        "description": (
            "Anomalies with seasonal cycle removed, "
            "before mean subtraction and normalization."
        )
    }
    ...
    std_data.attrs = {
        "description": (
            "Standard deviations of anomalies with seasonal cycle removed, "
            "before mean subtraction and normalization."
        )
    }
...
def retrieve_CEOFs_xeofs(ceof_model, vars_order=["ua850", "ua200", "rlut"]):
    ...
    pc_xeofs = (
        (pc_xeofs / np.sqrt(eigenvalues_xeofs))
        .assign_coords(mode=[0, 1])
        .transpose("time", "mode")
    )
    pc_xeofs.loc[{"mode": 0}] *= -1
...
def perform_CEOF_analysis(data=None, ceof_model=None, n_modes=2, vars_order=["ua850", "ua200", "rlut"]):
    ...
    vars_coords = np.array(
        vars_order, dtype=f"U{max(len(v) for v in vars_order)}"
    )  # Correct variables dimension format
    ceof_analysis_data = xr.Dataset({
...
def correct_CEOFs(ceof_new, ceof_ref, n_modes=2):
    ...
    corr_matrix = np.array([
            [xr.corr(ceof_ref_var.sel(mode=i), ceof_var.sel(mode=j)) for j in range(n_modes)]
    ...
    if swap > keep:
        ceof_corrected["ceof"].loc[{"mode": 0}] = ceof_new["ceof"].sel(mode=1)
        ceof_corrected["eigval"].loc[{"mode": 0}] = ceof_new["eigval"].sel(mode=1)
        ceof_corrected["var_frac"].loc[{"mode": 0}] = ceof_new["var_frac"].sel(mode=1)
        ceof_corrected["pc"].loc[{"mode": 0}] = ceof_new["pc"].sel(mode=1)

        ceof_corrected["ceof"].loc[{"mode": 1}] = ceof_new["ceof"].sel(mode=0)
        ceof_corrected["eigval"].loc[{"mode": 1}] = ceof_new["eigval"].sel(mode=0)
        ceof_corrected["var_frac"].loc[{"mode": 1}] = ceof_new["var_frac"].sel(mode=0)
        ceof_corrected["pc"].loc[{"mode": 1}] = ceof_new["pc"].sel(mode=0)
    ...
    if corr_matrix[0, 0] < 0:
        ceof_corrected["ceof"].loc[{"mode": 0}] *= -1
        ceof_corrected["pc"].loc[{"mode": 0}] *= -1
    if corr_matrix[1, 1] < 0:
        ceof_corrected["ceof"].loc[{"mode": 1}] *= -1
        ceof_corrected["pc"].loc[{"mode": 1}] *= -1
...
def compute_CEOFs_corr(ceof_1, ceof_2, n_modes=2):
    ...
    for var in ceof_1["variable"].values:
        ...
        for mode in range(n_modes):
            corr = xr.corr(
                ceof_1.sel(variable=var, mode=mode), ceof_2.sel(variable=var, mode=mode)
            )
...
def compute_climatological_phase_counts(pcs, threshold=None):
    ...
    for phase in range(1, 9):
        ...
        if phase_mask.sum() > 0:
            ...
            mean_active_amp = (
                amplitude.where(phase_mask & active_days)
                .resample(time="1YS")
                .mean()
                .mean(dim="time")
                .values.item()
    ...
    yearly_total_counts = (
        phase_pandas.groupby(phase_pandas.index.year)
        .value_counts()
    ...
    yearly_active_counts = (
        phase_active_pandas.groupby(phase_active_pandas.index.year)
        .value_counts()
        .unstack(fill_value=0)
        .sort_index(axis=1)
    )
