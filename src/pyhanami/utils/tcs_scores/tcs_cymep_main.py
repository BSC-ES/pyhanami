...
def write_configs_line(config, output_path):
    """
    Write a single configuration line to the configuration .csv file necessary for the CyMeP package.
    ...
    if not isinstance(config[0], str):
        raise ValueError(
            "The first element of the configuration must be a string (trajectories' filename)."
        )
    ...
    if not isinstance(config[3], int):
        raise ValueError(
            "The fourth element of the configuration must be an integer (nº ensemble members)."
        )
    if not isinstance(config[4], int):
        raise ValueError(
            "The fifth element of the configuration must be an integer (years per ensemble member)."
        )
    if not isinstance(config[5], (int, float)):
        raise ValueError(
            "The sixth element of the configuration must be a number (wind speed correction factor)."
        )

    # Write line to configs.csv file
    params = [str(item) for item in config]
...
def prepare_configs_file(configs, output_path=config_params.CYMEP_CONFIGS_PATH):
    ...
    # Add configuration for each dataset
    for dataset in configs:
        write_configs_line(configs[dataset], output_path)
...
def run_cymep(styr, enyr, output_path, gridsize=2.5, basin=-1, csvfilename=config_params.CYMEP_CONFIGS_PATH,
              truncate_years=True, do_defineMIbypres=False, do_fill_missing_pw=True,
              do_special_filter_obs=False, THRESHOLD_ACE_WIND=-1.0, THRESHOLD_PACE_PRES=-100.0,
              debug_level=0):
    """
    ...
    for ii in range(len(files)):
        ...
        # USER_MODIFY
        nstorms, ntimes, ncol, traj_data = tcs_cymep_funcs.getTrajectories(
        ...
        if quadratic_fit:
            ...
            xwindtmp = (
                quad_a[2] + quad_a[1] * (1010.0 - xpres) + quad_a[0] * ((1010.0 - xpres) ** 2)
            )
        ...
        storm_level_data = np.column_stack(
            (xgyear, xgmonth, xgday, xghour, xglat, xglon, xmpres, xmwind, xtcd, xace, xpace)
        )
        ...
        np.savetxt(
            output_path / f"storms_{strbasin}_{strs[ii]}_output.csv",
            filtered_storm_data,
            delimiter=",",
            fmt=data_formatting,
            header=header_str,
        ...
        for jj in range(styr, enyr + 1):
            ...
            if jj >= np.nanmin(xgyear) and jj <= np.nanmax(xgyear):
                ...
                pydict["py_tcd"][ii, yrix] = (
                    np.nansum(np.where(xgyear == jj, xtcd, 0.0)) / ensmembers[ii]
                )
                pydict["py_ace"][ii, yrix] = (
                    np.nansum(np.where(xgyear == jj, xace, 0.0)) / ensmembers[ii]
                )
                pydict["py_pace"][ii, yrix] = (
                    np.nansum(np.where(xgyear == jj, xpace, 0.0)) / ensmembers[ii]
                )
                pydict["py_lmi"][ii, yrix] = np.nanmean(
                    np.where(xgyear == jj, xlatmi, float("NaN"))
                )
                pydict["py_latgen"][ii, yrix] = np.nanmean(
                    np.where(xgyear == jj, np.absolute(xglat), float("NaN"))
                )
    ...
    for ii in range(nfiles):
        rxydict["rxy_track"][ii] = tcs_cymep_funcs.pattern_cor(
            msdict["fulldens"][0, :, :], msdict["fulldens"][ii, :, :], denslatwgt, 0
        )
        rxydict["rxy_gen"][ii] = tcs_cymep_funcs.pattern_cor(
            msdict["fullgen"][0, :, :], msdict["fullgen"][ii, :, :], denslatwgt, 0
        )
        rxydict["rxy_u10"][ii] = tcs_cymep_funcs.pattern_cor(
            msdict["fullwind"][0, :, :], msdict["fullwind"][ii, :, :], denslatwgt, 0
        )
        rxydict["rxy_slp"][ii] = tcs_cymep_funcs.pattern_cor(
            msdict["fullpres"][0, :, :], msdict["fullpres"][ii, :, :], denslatwgt, 0
        )
        rxydict["rxy_ace"][ii] = tcs_cymep_funcs.pattern_cor(
            msdict["fullace"][0, :, :], msdict["fullace"][ii, :, :], denslatwgt, 0
        )
        rxydict["rxy_pace"][ii] = tcs_cymep_funcs.pattern_cor(
    ...
    for ii in range(nfiles):
        ratio = tcs_cymep_funcs.taylor_stats(
    ...
    for ii in range(nfiles):
        taydict["tay_bias2"][ii] = 100.0 * (
            (taydict["uclim_count"][ii] - taydict["uclim_count"][0]) / taydict["uclim_count"][0]
        )

    # Save results to .csv and .nc files
    # Write out primary stats files
    tcs_cymep_funcs.write_single_csv(
        rxydict, strs, output_path, f"metrics_{strbasin}_spatial_corr.csv"
    )
    tcs_cymep_funcs.write_single_csv(
        rsdict, strs, output_path, f"metrics_{strbasin}_temporal_scorr.csv"
    )
    tcs_cymep_funcs.write_single_csv(
        rpdict, strs, output_path, f"metrics_{strbasin}_temporal_pcorr.csv"
    )
    tcs_cymep_funcs.write_single_csv(
        aydict, strs, output_path, f"metrics_{strbasin}_climo_mean.csv"
    )
    tcs_cymep_funcs.write_single_csv(
        asdict, strs, output_path, f"metrics_{strbasin}_storm_mean.csv"
    )
    tcs_cymep_funcs.write_single_csv(
        stdydict, strs[0], output_path, f"means_{strbasin}_climo_mean.csv"
    )
    ...
    # Write NetCDF file
    _ = tcs_cymep_funcs.write_spatial_netcdf(
...
def run_cymep_pyhanami(styr, enyr, output_path, gridsize=2.5, basin=-1,
                       csvfilename=config_params.CYMEP_CONFIGS_PATH, truncate_years=True,
                       do_defineMIbypres=False, do_fill_missing_pw=True,
                       do_special_filter_obs=False, THRESHOLD_ACE_WIND=-1.0,
                       THRESHOLD_PACE_PRES=-100.0, debug_level=0):
    """
    ...
    try:
        ...
        raise FileNotFoundError(f"Configuration file {csvfilename} not found.") from e
    ...
    for ii in range(len(files)):
        ...
        for jj in range(1, 12 + 1):
            ...
            pmdict["per_month_tcd"][ii, jj - 1] = (
                np.nansum(np.where(xgmonth == jj, xtcd, 0.0)) / nmodyears
            )
            pmdict["per_month_ace"][ii, jj - 1] = (
                np.nansum(np.where(xgmonth == jj, xace, 0.0)) / nmodyears
            )
            pmdict["per_month_pace"][ii, jj - 1] = (
                np.nansum(np.where(xgmonth == jj, xpace, 0.0)) / nmodyears
            )
            pmdict["per_month_lmi"][ii, jj - 1] = np.nanmean(
                np.where(xgmonth == jj, xlatmi, float("NaN"))
            )
        ...
        for jj in range(styr, enyr + 1):
            yrix = jj - styr  # Convert from year to zero indexing for numpy array
            if jj >= np.nanmin(xgyear) and jj <= np.nanmax(xgyear):
                pydict["per_year_count"][ii, yrix] = (
                    np.count_nonzero(xgyear == jj) / ensmembers[ii]
                )
                pydict["per_year_tcd"][ii, yrix] = (
                    np.nansum(np.where(xgyear == jj, xtcd, 0.0)) / ensmembers[ii]
                )
                pydict["per_year_ace"][ii, yrix] = (
                    np.nansum(np.where(xgyear == jj, xace, 0.0)) / ensmembers[ii]
                )
                pydict["per_year_pace"][ii, yrix] = (
                    np.nansum(np.where(xgyear == jj, xpace, 0.0)) / ensmembers[ii]
                )
                pydict["per_year_lmi"][ii, yrix] = np.nanmean(
                    np.where(xgyear == jj, xlatmi, float("NaN"))
                )
        ...
        if ii == 0:
            ...
            msvars = [
                ...
                "spatial_bias_ace",
                "spatial_bias_pace",
                "spatial_bias_minpres",
                "spatial_bias_maxwind",
                "spatial_bias_gen",
            ]
            for x in msvars:
                msdict[x] = np.empty((nfiles, denslat.size, denslon.size))
        ...
        msdict["spatial_bias_count"][ii, :, :] = (
            trackdens[:, :] - msdict["spatial_abs_count"][0, :, :]
        )
        msdict["spatial_bias_tcd"][ii, :, :] = tcddens[:, :] - msdict["spatial_abs_tcd"][0, :, :]
        msdict["spatial_bias_ace"][ii, :, :] = acedens[:, :] - msdict["spatial_abs_ace"][0, :, :]
        msdict["spatial_bias_pace"][ii, :, :] = (
            pacedens[:, :] - msdict["spatial_abs_pace"][0, :, :]
        )
        msdict["spatial_bias_minpres"][ii, :, :] = (
            minpres[:, :] - msdict["spatial_abs_minpres"][0, :, :]
        )
        msdict["spatial_bias_maxwind"][ii, :, :] = (
            maxwind[:, :] - msdict["spatial_abs_maxwind"][0, :, :]
        )
    ...
    rxyvars = [
        ...
        "spatial_pcorr_ace",
        "spatial_pcorr_pace",
        "spatial_pcorr_minpres",
        "spatial_pcorr_maxwind",
        "spatial_pcorr_gen",
    ]
    for x in rxyvars:
        rxydict[x] = np.empty(nfiles)
    ...
    for ii in range(nfiles):
        rxydict["spatial_pcorr_count"][ii] = tcs_cymep_funcs.pattern_cor(
            msdict["spatial_abs_count"][0, :, :],
            msdict["spatial_abs_count"][ii, :, :],
            denslatwgt,
            0,
        )
        rxydict["spatial_pcorr_tcd"][ii] = tcs_cymep_funcs.pattern_cor(
            msdict["spatial_abs_tcd"][0, :, :], msdict["spatial_abs_tcd"][ii, :, :], denslatwgt, 0
        )
        rxydict["spatial_pcorr_ace"][ii] = tcs_cymep_funcs.pattern_cor(
            msdict["spatial_abs_ace"][0, :, :], msdict["spatial_abs_ace"][ii, :, :], denslatwgt, 0
        )
        rxydict["spatial_pcorr_pace"][ii] = tcs_cymep_funcs.pattern_cor(
            msdict["spatial_abs_pace"][0, :, :],
            msdict["spatial_abs_pace"][ii, :, :],
            denslatwgt,
            0,
        )
        rxydict["spatial_pcorr_minpres"][ii] = tcs_cymep_funcs.pattern_cor(
            msdict["spatial_abs_minpres"][0, :, :],
            msdict["spatial_abs_minpres"][ii, :, :],
            denslatwgt,
            0,
        )
        rxydict["spatial_pcorr_maxwind"][ii] = tcs_cymep_funcs.pattern_cor(
            msdict["spatial_abs_maxwind"][0, :, :],
            msdict["spatial_abs_maxwind"][ii, :, :],
            denslatwgt,
            0,
        )
        rxydict["spatial_pcorr_gen"][ii] = tcs_cymep_funcs.pattern_cor(
            msdict["spatial_abs_gen"][0, :, :], msdict["spatial_abs_gen"][ii, :, :], denslatwgt, 0
        )
    ...
    for ii in range(nfiles):
        taydict["tay_bias2"][ii] = 100.0 * (
            (acdict["clim_mean_count"][ii] - acdict["clim_mean_count"][0])
            / acdict["clim_mean_count"][0]
        )
    ...
    metrics_descriptions = {
        ...
        **{
            ...
            for metric in data_metrics
            if data_metrics[metric]["temporal"] == True and metric != "count"
        },
    ...
    data_cymep = tcs_cymep_funcs.write_cymep_output_pyhanami(
        pmdict,
        pydict,
        acdict,
        asdict,
