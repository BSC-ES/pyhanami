...
class DataChecker:
    ...
    def normalize_units(unit_str):
        ...
        for part in parts:
            ...
            for i, c in enumerate(part):
                if (c == "-" and part[i - 1] not in {"^", "*"}) or (
                    c.isdigit() and part[i - 1] not in {"-", "^", "*"}
                ):
                    new_part.append(f"**{c}")
                else:
                    new_part.append(c)
    ...
    def check_standard_compliance(self, data):
        ...
        for coord in ["lat", "lon"]:
            ...
            if coord_values.ndim != 1:
                self.error_msg.append(f"'{coord}' coordinate must be a 1D array. ")
        ...
        for var in data.data_vars:
            if var not in self.variables:
                self.error_msg.append(f"Variable '{var}' not found in  'variables.yaml'. ")
                ...
                if "units" not in var_attrs:
                    self.error_msg.append(f"Variable '{var}' is missing a 'units' attribute. ")
        ...
        for var in data.data_vars:
            if var in self.variables:
                ...
                if nan_count == total_count:
                    self.error_msg.append(f"Variable '{var}' only contains NaN values.")
                    all_nan = True
    ...
    def check_temporal_completeness(self, data):
        ...
        if inferred_freq is None:
            ...
            expected_times = pd.date_range(
                start=time_index[0], end=time_index[-1], freq=inferred_freq
            )
                    ...
                    if "boundaries" in expected_var:
                        if expected_var["boundaries"] is not None:
                            self.error_msg.append(
                                ...
                                "No physical plausibility check has been implemented for this case yet."
                            )
    ...
    def check_data(self, data):
        ...
        if len(self.warning_msg) != 0:
            print(
                f"{len(self.warning_msg)} warnings encountered while checking the provided dataset:",
                flush=True,
            )
