...
def parse_value(value, expected_type_str):
    ...
    # Handle structured types
        ...
        try:
            ...
            aux_value = eval(value, {"__builtins__": {}}, {})

        except Exception as e:
            raise TypeError(
                f"Value '{value}' cannot be parsed as type '{expected_type_str}'."
            ) from e
    ...
    try:
        ...
        raise TypeError(
            f"Value '{value}' cannot be parsed as type '{expected_type_str}'. Error: {e}"
        ) from e
...
def make_config_class(phenomenon):
    ...
    if phenomenon not in PARAMETERS:
        raise ValueError(
            f"Phenomenon '{phenomenon}' not found in '{config_params.SCI_EVAL_PARAMS_PATH}'. "
    ...
    for param_name, param_info in phenomenon_params.items():
        ...
        if "value" not in param_info or "type" not in param_info:
            raise ValueError(
                f"Parameter '{param_name} in phenomenon '{phenomenon}' must have 'value' and "
        ...
        if param_type_str not in TYPE_MAP:
            raise ValueError(
                f"Unsupported type '{param_type_str}' for parameter '{param_name}' in phenomenon "
        ...
        dataclass_fields.append((
                param_name,
                param_type,
                field(default=default_value, metadata={"description": param_info["description"]}),
        ))
    ...
    ConfigClass = make_dataclass(
        cls_name=class_name, fields=dataclass_fields, namespace={"__post_init__": __post_init__}
    )
