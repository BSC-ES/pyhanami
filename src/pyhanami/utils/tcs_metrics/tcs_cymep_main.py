...
def write_configs_line(config, output_path):
    ...
    if not isinstance(config[2], bool):
        raise ValueError("The third element of the configuration must be a boolean (unstructured trajectories file?, i.e. 1D column indices instead of 2D).")
    if not isinstance(config[3], int):
        raise ValueError("The fourth element of the configuration must be an integer (nº ensemble members).")
    if not isinstance(config[4], int):
        raise ValueError("The fifth element of the configuration must be an integer (years per ensemble member).")
    if not isinstance(config[5], (int, float)):
        raise ValueError("The sixth element of the configuration must be a number (wind speed correction factor).")
...
def run_cymep(styr, enyr, gridsize, output_path, basin=-1, csvfilename=config_params.CYMEP_CONFIGS_PATH, truncate_years=True,
    ...
    for ii in range(nfiles):
        rxydict['rxy_track'][ii] = tcs_cymep_funcs.pattern_cor(msdict['fulldens'][0,:,:], msdict['fulldens'][ii,:,:], denslatwgt, 0)
        rxydict['rxy_gen'][ii]   = tcs_cymep_funcs.pattern_cor(msdict['fullgen'][0,:,:],  msdict['fullgen'][ii,:,:],  denslatwgt, 0)
        rxydict['rxy_u10'][ii]   = tcs_cymep_funcs.pattern_cor(msdict['fullwind'][0,:,:], msdict['fullwind'][ii,:,:], denslatwgt, 0)
        rxydict['rxy_slp'][ii]   = tcs_cymep_funcs.pattern_cor(msdict['fullpres'][0,:,:], msdict['fullpres'][ii,:,:], denslatwgt, 0)
        rxydict['rxy_ace'][ii]   = tcs_cymep_funcs.pattern_cor(msdict['fullace'][0,:,:],  msdict['fullace'][ii,:,:],  denslatwgt, 0)
        rxydict['rxy_pace'][ii]  = tcs_cymep_funcs.pattern_cor(msdict['fullpace'][0,:,:], msdict['fullpace'][ii,:,:], denslatwgt, 0)
    ...
    for ii in range(nfiles):
        ratio = tcs_cymep_funcs.taylor_stats(msdict['fulldens'][ii,:,:], msdict['fulldens'][0,:,:], denslatwgt,0)
    ...
    for ii in range(nfiles):
        taydict["tay_bias2"][ii] = 100. * ( (taydict['uclim_count'][ii] - taydict['uclim_count'][0]) / taydict['uclim_count'][0] )
    ...
    tcs_cymep_funcs.write_single_csv(aydict, strs, output_path, f'metrics_{strbasin}_climo_mean.csv')
