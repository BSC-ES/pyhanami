...
When running an ESM, the surface of the Earth is represented as a grid composed of multiple cells. We use the climatologies (mean values over a span of time) of climate variables at each of these cells to compute the scores for the test. Some of the variables used are the atmospheric air temperature, the precipitation rate, the mean sea level pressure, or the the sea surface salinity. Directly averaging the values of these variables spatially would result in the loss of important information regarding regional differences among the ensemble members. Due to this, we first apply various metrics at the cell level, which compare each member's value against either the mean of the entire simulated ensemble or the mean of a common reference observational dataset. By taking the weighted spatial mean of these cell-level metrics, we obtain scalar scores. 

Particularly, we combine the outcome of three different scores and evaluate them with four distinct statistical tests. Before applying the statistical tests to each pair of score distributions, we check whether the effect size between the distributions exceeds a threshold defined by the minimum detectable effect size for a _t_-test at the chosen significance level and statistical power. This threshold depends on the sample sizes (i.e., the number of ensemble members) in the two score distributions being compared. If the effect size is below this threshold, we conclude that the sample sizes are too small to reliably detect meaningful differences (insufficient statistical power), and the null hypothesis is automatically rejected. Otherwise, we apply the statistical tests, and we reject the null hypothesis if at least one of the tests results in a rejection, indicating that there is a significant difference between the distributions.
...
- **MJO activity per phase:** climatological (annually averaged) values of the mean amplitude and the number of active days per phase. 
