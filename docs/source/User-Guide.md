...
- **Variables:** for each variable to be analyzed, ensure it is defined in `src/pyhanami/config/variables.yaml` following CMIP conventions (see [Configuration](./Configuration.md#variablesyaml)). It is required that each variable (as a xarray.DataArray) has as attribute the corresponding `units`.
- **Scientific evaluation parameters:** for each climate phenomenon to be evaluated, the relevant parameters and their default values are defined in `src/pyhanami/config/scientific_evaluation_parameters.yaml` (see [Configuration](./Configuration.md#scientific_evaluation_parametersyaml)). Ensure that the default values for the parameters are appropriate for your analysis. If necessary, modify them permanently in the configuration file or temporarily when calling the corresponding method (see [Configuration of scientific evaluation parameters](./User-Guide.md#configuration-of-scientific-evaluation-parameters)). 
- **Tropical Cyclone (TC) metrics:** for TC evaluation, each computed metric and its associated metadata are defined in `src/pyhanami/config/tc_metrics.yaml` (see [Configuration](./Configuration.md#tc_metricsyaml)). These entries can be modified to control how metrics are displayed in tables and plots. If you change the default values, verify that the selected score types (`temporal`, `spatial`) are scientifically meaningful for each metric.
- **Package default constants (advanced):** package-level paths and internal default values are defined in `src/pyhanami/config/config_params.py` (see [Configuration](./Configuration.md#config_paramspy-internal)). In most cases, you should **leave these values unchanged**. Advanced users may edit them to adapt local setups (for example, changing default reference data paths or adjusting default parallelization limits).
